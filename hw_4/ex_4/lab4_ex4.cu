...
//leaner function (no print output and no input arguments required)
double cputimer_getDuration(){
  gettimeofday(&t_end, 0);
...
int main(int argc, char **argv) {
  ...
  // allocate managed mem with gpuCheck
  ...
  cusparseCheck(cusparseSpMV_bufferSize(cusparseHandle, CUSPARSE_OPERATION_NON_TRANSPOSE, &one, Adescriptor,
    tempDescriptor, &zero, tmpDescriptor, CUDA_R_64F, CUSPARSE_SPMV_CSR_ALG1, &bufferSize));

  //@@ Insert code to allocate the buffer needed by cuSPARSE
  gpuCheck(cudaMalloc(&buffer, bufferSize));

  // Init time required for all for loop iterations
  ...
  for (int it = 0; it < nsteps; ++it) {
    ...
    cusparseCheck(cusparseSpMV(cusparseHandle, CUSPARSE_OPERATION_NON_TRANSPOSE, &one, Adescriptor,
      tempDescriptor,&zero, tmpDescriptor, CUDA_R_64F, CUSPARSE_SPMV_CSR_ALG1, buffer));
    total_time += cputimer_getDuration();
  ...
  printf("Timing - Computing the SMPV. \t\t\tElasped %.0f microseconds \n", total_time);
